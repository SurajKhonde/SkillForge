...
import PosterSelector from "../../CommonComponents/PosterSelector";
...
export default function HrForm({
  ...
  const [hrinfo, setHrInfo] = useState({ ...defaultActorInfo });
  ...
  const handleChange = ({ target }) => {
    ...
    if (name === "avatar") {
      ...
      return setHrInfo({ ...hrinfo, avatar: file });
    }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { error } = validateActor(hrinfo);
    if (error) return updateNotification("error", error);

    // submit form
    const formData = new FormData();
    for (let key in hrinfo) {
      if (key) formData.append(key, hrinfo[key]);
    }
    onSubmit(formData);
  ...
  useEffect(() => {
    if (initialState) {
      setHrInfo({ ...initialState, avatar: null });
      setSelectedAvatarForUI(initialState.avatar);
    }
  }, [initialState]);

  const { name, about, gender,companyName } = hrinfo;
  return (
    <form
